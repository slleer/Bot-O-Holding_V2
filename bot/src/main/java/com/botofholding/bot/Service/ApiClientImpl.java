...
public class ApiClientImpl implements ApiClient {
    ...
    @Override
    public Mono<ApiResponsePayload<BohUserSummaryDto>> getMyProfile() {
        logger.debug("Attempting to retrieve current user's profile from /api/users/me");
        return webClient.get()
                .uri("/users/me")
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<StandardApiResponse<BohUserSummaryDto>>() {})
                .flatMap(this::handleApiResponseWithPayload);
    }

    @Override
    public Mono<ApiResponsePayload<BohUserSummaryDto>> updateMyProfile(UserRequestDto dto) {
